...
pub async fn copy_folder_inner(
    ...
    let list_children_config: list::ListFilesConfig = list::ListFilesConfig {
        query: list::ListQuery::FilesInFolder { folder_id: config.to_folder_id.clone() },
        order_by: list::ListSortOrder::FolderModifiedName,
        max_files: 1000,
    };

    let children = list::list_files(hub, &list_children_config).await
    ...
    for child in children {
        if drive_file::is_directory(&child) {
            ...
            let new_folder = mkdir::create_directory(hub,&mkdir_config, delegate_config.clone() ).await.map_err(Error::MKDirError)?;
            let new_folder_config = 
                CopyFolderConfig { 
                    src_folder_id: child.id.ok_or(Error::FileWithoutId)?,
                    to_folder_id: new_folder.id.ok_or(Error::FileWithoutId)?
                };
            Box::pin(copy_folder_inner(hub, delegate_config.clone(), &new_folder_config)).await?;
        } else {
            let copy_config = copy::CopyConfig {
                file_id: child.id.ok_or(Error::FileWithoutId)?,
                to_folder_id: config.to_folder_id.clone(),
            };
            copy::copy_file(hub, delegate_config.clone(), &copy_config).await.map_err(Error::Copy)?;
        }
    }
